...
<div class="offcanvas offcanvas-start" tabindex="-1" id="offcanvasExample" aria-labelledby="offcanvasExampleLabel">
	<div class="offcanvas-header">
		...
		<button type="button" class="btn-close btn-ch text-reset" data-bs-dismiss="offcanvas" aria-label="Close"></button>
	</div>
	<div class="offcanvas-body">
		<div class="row">
			...
			<div class="col-auto text-end">
				<button class="btn btn-ch btnDeleteIcon" type="button" title="편집">
		...
		<div class="collapse" id="collapseExample">
		  <div class="card card-body">
		  	<div class="input-group">
				...
				<button type="button" class="btn btn-ch btn-outline-success btnCategoryAddOk"><i class="bi bi-save"></i></button>
			</div>
		  </div>
		</div>
		...
		<c:if test="${listCategory.size() > 0}">
			<div class="row">
				<div class="col pt-1 text-end">
					<button type="button" class="btn btn-ch btnCategorySearch" title="검색"><i class="bi bi-search"></i></button>
				</div>
			</div>
		</c:if>
		...
		<div class="modal-content">
			...
			<div class="modal-body pt-1">
				...
				<table class="table table-borderless">
					<tr>
						<td class="text-end">
							<button type="button" class="btn btn-outline-primary btnScheduleUpdate">상세 수정</button>
			    			<button type="button" class="btn btn-outline-danger btnScheduleDelete">예약 삭제</button>
						</td>
					</tr>
				</table>
			</div>
...
function insertSchedule(startStr, endStr, allDay) {
	...
	if(allDay) {
		query = "sday="+startStr;
		if(endStr) {
			//시간일정 안되고 전체일정일때만 .강사님이 만드신 js dayLater함수
	...
	location.href="${pageContext.request.contextPath}/hostPage/calendarWrite?"+query;
}
