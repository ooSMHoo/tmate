...
public class MyUtilGeneral extends MyUtil {
	...
	public String pagingMethod2(int current_page, int total_page, String methodName, String option) {
        ...
        n = current_page - numPerBlock;
        if ((total_page > numPerBlock) && (currentPageSetUp > 0)) {
			sb.append("<a onclick=\"" + methodName + "(1, '"+option+"');\">처음</a>");
			sb.append("<a onclick=\"" + methodName + "(" + n + ", '"+option+"');\">이전</a>");
        }
        ...
        while((page <= total_page) && (page <= currentPageSetUp + numPerBlock)) {
           if(page == current_page) {
			   ...
			   sb.append("<a onclick=\"" + methodName + "(" + page +", '"+option+"');\">" + page + "</a>");
           }
           page++;
        }
		...
		if(n > total_page) n = total_page;
        if (total_page - currentPageSetUp > numPerBlock) {
			sb.append("<a onclick=\"" + methodName + "(" + n + ", '"+option+"');\">다음</a>");
			sb.append("<a onclick=\"" + methodName + "(" + total_page + ", '"+option+"');\">끝</a>");
        }
		sb.append("</div>");
